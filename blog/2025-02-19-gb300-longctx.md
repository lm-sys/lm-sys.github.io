...
by: Nvidia & SGLang Team, Feb XX, 2026
...
### **2. Prefill path: PP prefill, long-context TTFT, and stronger kernels**
...
The computation of KV footprint: (kv_lora_rank + qk_rope/head_dim) × num_layers × num_kv_head * fp8_size = (512 + 64) × 61 × 1 × 1 = 35,136 Bytes per token</small>
...
As the table shows, GB300's larger HBM translates almost directly into higher decode concurrency — with theoretical caps at 40 vs 24 req/GPU. In practice, we operate at ~85% of the cap to preserve headroom for runtime allocations and CUDA graph capture, yielding a maximum of 36 req/GPU on GB300 versus 20 on GB200 (i.e., 576 vs 320 concurrent requests at DEP16), without causing unnecessary retraction.


### **4. MTP powered by overlap scheduler**
...
4.2 Peak Capacity vs. Latency Constraints
...
Latency–throughput balanced scenario (TPS/User = 50–60 for Non-MTP, 60–70 for MTP), which reflects higher per-user load and stricter responsiveness requirements.
...
Overall, GB300 consistently outperforms GB200 by 1.4X–1.5X, with the largest relative gains under latency-sensitive conditions where GB300 shows stronger resilience to throughput degradation at higher per-user load.
4.3 Prefill Latency: Chunking Strategies and TTFT Optimization
...
Key Highlight: 1.35X Faster Attention Kernel
...
205ms
GB200
fmhaSm100fKernel_QkvE4m3OBfloat16HQk192HV128SeparateQkvCausalVarSeqQ128Kv128PersistentContext
277ms
