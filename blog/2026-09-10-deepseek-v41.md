...
Random access to large tables can be limited by address translation. Huge-page backing reduces this overhead. In the evaluated GB300 container, host-sharded tables used anonymous mappings that supported huge pages, while shared mappings did not. These runs therefore used host sharding, retaining the all-reduce in exchange for faster host lookups. The best placement depends on CPU–GPU connectivity, huge-page availability, and workload.
...
To place Engram tables in host memory, set `SGLANG_ENABLE_DSV41_ENGRAM_HOST_TABLE=1`; `SGLANG_DSV41_ENGRAM_HOST_TABLE_LAYOUT` selects the `shared` (default) or `per_rank` layout.

![Engram layouts: GPU-sharded and host-sharded tables retain the lookup all-reduce; shared host tables remove it](/images/blog/deepseek-v41/engram-host-table.svg)
