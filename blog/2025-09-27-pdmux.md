...
previewImg: /images/blog/pdmux/logo.svg
---
...
In particular, when targeting an **ITL SLO below 100ms**—a critical threshold for practical LLM services—the shortcomings of both disaggregation and chunking become increasingly pronounced.

## PD-Multiplexing: A New Serving Paradigm

<img src="/images/blog/pdmux/1-overview.png" style="display:block; margin-top: auto; margin-left: auto; margin-right: auto; margin-bottom: auto; width: 75%; image-orientation: none;"></img>
...
<img src="/images/blog/pdmux/2-mux-engine.png" style="display:block; margin-top: auto; margin-left: auto; margin-right: auto; margin-bottom: auto; width: 75%; image-orientation: none;"></img>
...
With the bubble-less multiplex engine in place, the next challenge is scheduling prefill blocks and decode batches. Offline profiling shows that the two phases compete for resources under GreenContext. The root cause is that while GreenContext partitions SMs, it does not partition memory bandwidth, making contention difficult to model. To address this, we profile representative workloads offline and use the results to train a latency predictor that drives our SLO-aware scheduling policies.

The intuition behind the policy is simple: allocate just enough SMs to the decode phase to guarantee the ITL SLO, then dedicate all remaining SMs to prefill. At the same time, we determine the number of prefill blocks to launch. This way, decode always runs under strict SLO guarantees, while prefill proceeds as quickly as possible to enlarge the decode batch size.

Because both the modeling and policy are tied to the deployed model and hardware environment, we will release a detailed tutorial with practical, step-by-step guidance in the future.

## Benchmark

In summary, we set the ITL SLO target to 100 ms and evaluate PD-Multiplexing against multiple baselines across a range of workloads and devices. While no explicit SLO is enforced for prefill, we compare its tail latency to highlight the effectiveness of PD-Multiplexing. We first present an experiment that is easy to reproduce, then demonstrate the advantages of PD-Multiplexing using real-world traces and diverse tasks. Finally, we provide a zoomed-out visualization of runtime scheduling details. In our extensive evaluations, PD-Multiplexing improves goodput by up to 3.06x over state-of-the-art baselines.

### Comparison with Chunked-prefill under Varying Chunk Sizes

<img src="/images/blog/pdmux/3-H200.png" style="display:block; margin-top: auto; margin-left: auto; margin-right: auto; margin-bottom: auto; width: 60%; image-orientation: none;"></img>
...
We first evaluate PD-Multiplexing against chunked-prefill with varying chunk sizes on a single H200 GPU running CodeLlama-34b-hf. For fairness, prefix cache sharing is disabled. Figure 3 reports the 99th-percentile TTFT and ITL. The results highlight a clear advantage: PD-Multiplexing reduces both TTFT and ITL simultaneously, whereas chunked-prefill suffers from a seesaw effect—improving one metric at the expense of the other. This benefit becomes even more pronounced for long-context workloads such as LooGLE, where the inefficiency of chunking is magnified. Reproduction scripts are available [here.](https://github.com/sgl-project/sglang/pull/10692)
...
<img src="/images/blog/pdmux/4-mooncake.png" style="display:block; margin-top: auto; margin-left: auto; margin-right: auto; margin-bottom: auto; width: 60%; image-orientation: none;"></img>
...
<img src="/images/blog/pdmux/5-open-share-loogle.png" style="display:block; margin-top: auto; margin-left: auto; margin-right: auto; margin-bottom: auto; width: 60%; image-orientation: none;"></img>
...
<img src="/images/blog/pdmux/6-visualization.png" style="display:block; margin-top: auto; margin-left: auto; margin-right: auto; margin-bottom: auto; width: 60%; image-orientation: none;"></img>
...
* **Speculative inference.** Speculative decoding involves multiple collaborating LLMs, which calls for a different resource-partitioning strategy. We will extend PD-Multiplexing with policies tailored to multi-model pipelines and report end-to-end gains.
...
- We would like to thank Panyi for the insight discussions during the early stages of this work.