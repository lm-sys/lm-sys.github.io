...
date: "June 29, 2023"
previewImg: /images/blog/longchat/topic_retrieval.png
---

In this blogpost, we introduce our latest series of chatbot models, LongChat, featuring a new level of extended context length. 
...
The data and code used to reproduce the results in the blog post are available in this notebook.

## Training Recipe - LongChat
...
The LLaMA model is pre-trained with rotary embedding on sequence length 2048, which means that it has not observed scenarios where position_ids > 2048 during the pre-training phase. Instead of forcing the Llama model to adapt to position_ids > 2048, we instead condense position_ids > 2048 to be within 0 to 2048. Intuitively, we conjecture this condensation can maximally reuse the model weights learned in the pre-training stage.

Concretely, we define a term condensation ratio by dividing the target new context length y by 2048. We then divide every position_ids by this ratio and feed it into the apply_rotary_pos_emb() function. Conceptually, this is one line of code change:
```python
query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids / ratio)
```
In this release, we fine-tune the model to y=16384, and thus the condensation ratio is 8. For instance, a token with position_ids = 10000 becomes position_ids = 10000 / 8 = 1250, and the neighboring token 10001 becomes 10001 / 8 = 1250.125. This step of condensing requires no training.

### Step 2: Fine-tuning on Curated Conversation Dataset
After condensing the embedding, we perform the fine-tuning procedure on our curated conversation dataset. 
We reuse our collected user-shared conversations previously used for training Vicuna. 
We clean the data using FastChat, and split these conversations if they are longer than 16K. 
We fine tune the model using standard next-token prediction loss. We fine-tune the 7B and 13B model with 80k and 18k conversations respectively. 
To save memory, we use Pytorch FSDP and Flash Attention. Assume A100 is $3/hour on Cloud, the 7B model costs ~$300, and the 13B model costs approximately $700. 
...
We compare 6 recently released LLMs supporting long context, including four open source models and two proprietary models. Their specifications are listed in Table 1 below.
...
<tr> <td><a target="_blank" href="https://huggingface.co/mosaicml/mpt-30b-chat">MPT-30-chat</a></td>  <td>30B</td>  <td>Yes</td>  <td>8K</td>  <td>-</td> <td>16K</td> <td>Yes</td> </tr>
...
From the coarse-grained topic retrieval test results (Figure 2), we already observe false promises of open-source long-context models. For instance, Mpt-7b-storywriter claims to have a context length of 65K, but barely achieves 50% accuracy even at one fourth of its claimed context length (16K). 
...
In the finer-grained line retrieval test, Mpt-7b-storywriter performs even worse than in the coarse-grained cases, dropping accuracy from ~50% to ~30%. Chatglm2-6B also observes degradation, not being able to reliably retrieve the associated number at the shortest length we test (5K context length). In contrast, we observe that LongChat-13B-16K perform reliably, achieving near gpt-3.5/Anthropic-claude ability within 12K context length and decent performance towards its limit (The preview version is not perfect at 12K-16K, see discussion section).
...
In the previous section, we have observed that LongChat models perform well on long-range retrieval tasks, but does this come with a significant drop of human preference? To test whether it still follows human preferences, we use GPT-4 graded [MT-bench](https://github.com/lm-sys/FastChat/tree/main/fastchat/llm_judge), a set of challenging multi-turn conversation questions. We find that LongChat-13B-16K is comparable to its closest alternatives - Vicuna-13B, which indicates that this long-range ability does not come with a significant sacrifice of its short-range ability. At the same time, LongChat-13B-16K is competitive compared to other models of the same size.
...
## Conclusion
In our evaluations, commercial long-context models always fulfill its promise: gpt-3.5-16k and Anthropic Claude (almost) achieve perfect performance in both our benchmarks. However, existing open-sourced models often do not achieve the claimed context-length. For instance, ChatGLM2 can generate text with 32K context length due to the system support from Flash Attention, but it can not reliably perform long range retrieval beyond a much smaller context length (~6K). Mpt-7b-storywriter claims a 65k context length but can barely achieve 50% accuracy beyond a much smaller context length (10k for topic retrieval task and 5k for line retrieval task). We qualitatively summarize our assessment in the following table.
...
## Next Steps
Inspired by the promising performance and the simple training recipe of our 16K models, we would like to explore even longer chatbot models.

We have observed many efficiency issues (e.g. memory and throughput) during training/inferencing chatbots of much longer context length. We plan to develop system for long-context chatbot training/inference
...
Dacheng Li*, Rulin Shao*, Ying Sheng, Lianmin Zheng, Anze Xie, Ion Stoica, Xuezhe Ma, Hao Zhang
