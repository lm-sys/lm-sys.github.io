...
**Update on August 18, 2026:** [Session Server V2](https://miles.radixark.com/docs/user-guide/agentic-rollout) supports collecting token information for the trajectory trees created when agent harnesses spawn subagents or compact context. It performs best-effort sample merging and carries the corresponding loss masks through the merge. Because this merge is best-effort, a task may yield multiple training samples instead of always producing one sample per task instantiation. On the training side, Miles supports the resulting dynamic global batch size and normalizes the loss to keep gradient scale consistent.
