...
To assess the decode performance of DeepSeek on the GB200 NVL72, we conducted end-to-end experiments with a comparison to H100. In order to test peak decode throughput, we allocate most nodes for decode and use the slow-down API in the previous [blog post](https://lmsys.org/blog/2025-05-05-large-scale-ep/) to make decode nodes saturated. More specifically, we use 2 nodes for prefill and 12 nodes for decode to utilize the provided 60 GPUs in a GB200 NVL72. To ensure consistency, the experimental setup mirrors that of our previous blog with baselines reused directly from it. The yellow baseline bar indicates simulated MTP in the previous blog, and other bars do not enable MTP. Thus, the speedup is higher than displayed if using the same setup.
