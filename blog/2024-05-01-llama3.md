...
**Effect of duplicate questions.** Using fuzzy string matching, we find that ~9% (6658/7327) of the user prompts in battles between Llama 3 and the other top models are duplicates, and show in Table X that deduplication does not significantly affect Llama3 win-rate, and only changes Llama 3-70b win rates against the top 5 models in the arena with and without duplication. 
